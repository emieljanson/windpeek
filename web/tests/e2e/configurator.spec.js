...
test('keeps compact mode focused on direct display options', async ({ page }) => {
  ...
  await expect(reTerminalDialog.getByRole('link', { name: 'Buy reTerminal E1001' })).toHaveAttribute('href', /sensecap_affiliate=UF4PmgK/)
  await expect(reTerminalDialog.getByRole('link', { name: 'Buy reTerminal E1002' })).toHaveAttribute('href', /sensecap_affiliate=UF4PmgK/)
  await expect(reTerminalDialog.getByRole('link', { name: 'Buy reTerminal E1003' })).toHaveAttribute('href', /sensecap_affiliate=UF4PmgK/)
  await page.keyboard.press('Escape')
  await expect(reTerminalDialog).toHaveCount(0)
  await expect(reTerminalHelp).toBeFocused()
  const deviceSelect = page.getByRole('combobox', { name: 'reTerminal' })
  await expect(deviceSelect).toBeVisible()
  await expect(deviceSelect.locator('option')).toHaveText(['E1001', 'E1002', 'E1003'])
  await deviceSelect.selectOption('seeedstudio_reterminal_e1003')
  await expect(deviceSelect).toHaveValue('seeedstudio_reterminal_e1003')
  await expect(page.locator('[data-scene-status="ready"]')).toBeVisible({ timeout: CONFIGURATOR_READY_TIMEOUT_MS })
