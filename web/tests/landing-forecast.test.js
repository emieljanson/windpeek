...
import LandingForecast from '../src/components/LandingForecast.vue'
...
describe('landing hero nearby default', () => {
  ...
  it('uses a complete example for missing swell and switches back when live data returns', async () => {
    ...
    const wrapper = mount(LandingForecast, { global: { plugins: [pinia] } })
  ...
  it('renders the swell landing with wind first, compact wind and its own data request', async () => {
    window.history.replaceState({}, '', '/?site=swell')
    const pinia = createPinia()
    const store = useConfiguratorStore(pinia)
    ...
    await vi.waitFor(() => expect(renderer.renderPreviewForDisplay).toHaveBeenCalledWith(
      expect.objectContaining({ windSize: 1, swellSize: 2, moduleOrder: [0, 1, 2, 3, 4], showTemperature: true }),
      expect.anything(),
    ))
  ...
  it('falls back to the photo when a later forecast redraw fails', async () => {
    const pinia = createPinia()
    const store = useConfiguratorStore(pinia)
    for (const action of ['initializeForecast', 'initializeTide', 'initializeNearbyDefault']) {
      vi.spyOn(store, action).mockResolvedValue(false)
    }
    const renderer = { renderPreviewForDisplay: vi.fn(() => frame), dispose: vi.fn() }
    loadSharedRenderer.mockResolvedValueOnce(renderer)
    const wrapper = mount(LandingForecast, { global: { plugins: [pinia] } })
    await vi.waitFor(() => expect(wrapper.classes()).toContain('is-ready'))
    renderer.renderPreviewForDisplay.mockImplementation(() => { throw new Error('redraw failed') })
    store.forecastRevision++
    await vi.waitFor(() => expect(wrapper.classes()).not.toContain('is-ready'))
