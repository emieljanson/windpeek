...
export async function botWebhook(request, env) {
  ...
  if (!env.TELEGRAM_WEBHOOK_SECRET || !env.TELEGRAM_CHAT_ID || !env.PROJECT_ACTIVITY || !env.SUCCESS_RATE_LIMITER) return reply(503)
  ...
  try {
    ...
    if (update === undefined) return reply(413)
    if (!update || typeof update !== 'object' || Array.isArray(update)) return reply(400)
