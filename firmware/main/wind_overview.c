...
size_t wind_overview_first_spot(size_t page, size_t count) {
    size_t first = page * WIND_OVERVIEW_PAGE_SIZE;
    size_t last_full_start = count > WIND_OVERVIEW_PAGE_SIZE ? count - WIND_OVERVIEW_PAGE_SIZE : 0;
    return first < last_full_start ? first : last_full_start;
}
...
wind_touch_action_t wind_overview_hit_test(int x, int y, bool overview,
    ...
    if (!overview) {
        ...
        if (y <= 80) {
            ...
            while (action.day_index < 4 &&
                   x >= 12 + 775 * (int)(action.day_index + 1) / 5) ++action.day_index;
        }
        return action;
    }
    ...
    size_t index = wind_overview_first_spot(page, count) + row;
