...
static bool maintenance_enabled = false;
static TickType_t maintenance_deadline = 0;
...
void http_server_enable_maintenance(uint32_t duration_seconds)
{
    ...
    maintenance_deadline = xTaskGetTickCount() + pdMS_TO_TICKS(duration_seconds * 1000U);
    ESP_LOGI(TAG, "Local firmware maintenance enabled for %lu seconds",
...
bool http_server_is_maintenance_enabled(void)
{
    ...
    if (maintenance_enabled &&
        (int32_t) (maintenance_deadline - xTaskGetTickCount()) > 0) {
        return true;
    }
...
static bool require_maintenance_mode(httpd_req_t *req)
{
    if (http_server_is_maintenance_enabled()) {
        return true;
    }
    ...
    httpd_resp_sendstr(
        req,
        "{\"status\":\"error\",\"message\":\"Press WAKE to enable firmware uploads for 10 minutes\"}");
    return false;
}

static bool require_provisioning_mode(httpd_req_t *req)
{
    ...
    httpd_resp_set_status(req, "403 Forbidden");
    httpd_resp_set_type(req, "application/json");
    httpd_resp_sendstr(req, "{\"status\":\"error\",\"message\":\"Disabled outside provisioning mode\"}");
    return false;
}
...
{
    if (!require_provisioning_mode(req)) {
        return ESP_FAIL;
    }
    if (!system_ready) {
        httpd_resp_set_status(req, HTTPD_503);
...
static esp_err_t wind_preview_handler(httpd_req_t *req)
{
    FILE *file = fopen(WIND_DASHBOARD_PREVIEW_PATH, "rb");
    if (!file) {
        return httpd_resp_send_err(req, HTTPD_404_NOT_FOUND, "No wind dashboard rendered");
    }
...
static esp_err_t wind_status_handler(httpd_req_t *req)
{
    ...
    cJSON_AddNumberToObject(response, "perform_result", diagnostics.perform_result);
    cJSON_AddNumberToObject(response, "http_status", diagnostics.http_status);
    cJSON_AddNumberToObject(response, "parse_result", diagnostics.parse_result);
    cJSON_AddNumberToObject(response, "response_bytes", diagnostics.response_length);
    cJSON_AddBoolToObject(response, "too_large", diagnostics.too_large);
    cJSON_AddBoolToObject(response, "allocation_failed", diagnostics.allocation_failed);
...
static esp_err_t config_handler(httpd_req_t *req)
{
    ...
    if (req->method == HTTP_GET) {
        ...
        cJSON_AddBoolToObject(root, "access_token_set", access_token && access_token[0] != '\0');
        ...
        cJSON_AddBoolToObject(root, "http_header_value_set",
                              http_header_value && http_header_value[0] != '\0');
        ...
        cJSON_AddStringToObject(root, "openai_api_key", "");
        cJSON_AddStringToObject(root, "google_api_key", "");
        cJSON_AddBoolToObject(root, "openai_api_key_set", openai_key && openai_key[0] != '\0');
        ...
        ESP_LOGD(TAG, "Config %s request received (%d bytes)",
                 req->method == HTTP_PATCH ? "PATCH" : "POST", received);
...
{
    if (req->method != HTTP_GET && !require_provisioning_mode(req)) {
        return ESP_FAIL;
    }
    if (!system_ready) {
        httpd_resp_set_status(req, HTTPD_503);
...
static esp_err_t ota_status_handler(httpd_req_t *req)
{
    ...
    cJSON_AddBoolToObject(response, "local_upload_enabled",
                         http_server_is_maintenance_enabled());
...
{
    if (!require_maintenance_mode(req)) {
        return ESP_FAIL;
    }
    if (req->method == HTTP_POST) {
...
static esp_err_t ota_upload_handler(httpd_req_t *req)
{
    ...
    esp_err_t err = ota_begin_local_upload((size_t) req->content_len);
    if (err != ESP_OK) {
        httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Cannot start firmware upload");
        return ESP_FAIL;
    }

    uint8_t *buffer = malloc(8192);
    if (buffer == NULL) {
        ota_abort_local_upload();
        httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Out of memory");
        return ESP_FAIL;
    }
    ...
    while (remaining > 0) {
        int received = httpd_req_recv(req, (char *) buffer, MIN(remaining, 8192));
        if (received == HTTPD_SOCK_ERR_TIMEOUT && timeout_retries++ < 5) {
            continue;
        }
        if (received <= 0 || ota_write_local_upload(buffer, (size_t) received) != ESP_OK) {
            free(buffer);
            ota_abort_local_upload();
            httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR,
    ...
    httpd_resp_sendstr(req,
                       "{\"status\":\"success\",\"message\":\"Firmware validated; rebooting\"}");
    xTaskCreate(restart_task, "ota_restart", 4096, NULL, 5, NULL);
    return ESP_OK;
}
...
esp_err_t http_server_init(void)
{
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        httpd_uri_t wind_preview_uri = {.uri = "/api/wind-preview",
                                        .method = HTTP_GET,
                                        .handler = wind_preview_handler,
                                        .user_ctx = NULL};
        httpd_register_uri_handler(server, &wind_preview_uri);

        httpd_uri_t wind_status_uri = {.uri = "/api/wind-status",
                                       .method = HTTP_GET,
                                       .handler = wind_status_handler,
                                       .user_ctx = NULL};
        httpd_register_uri_handler(server, &wind_status_uri);
