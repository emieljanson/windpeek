...
static esp_ota_handle_t local_ota_handle = 0;
static const esp_partition_t *local_ota_partition = NULL;
static size_t local_ota_image_size = 0;
static size_t local_ota_bytes_written = 0;
...
static void set_ota_progress(int progress)
{
    if (ota_status_mutex && xSemaphoreTake(ota_status_mutex, portMAX_DELAY) == pdTRUE) {
...
esp_err_t ota_begin_local_upload(size_t image_size)
{
    if (ota_status_mutex == NULL || image_size == 0 || local_ota_handle != 0) {
        return ESP_ERR_INVALID_STATE;
    }
    if (ota_status.state == OTA_STATE_CHECKING || ota_status.state == OTA_STATE_DOWNLOADING ||
        ota_status.state == OTA_STATE_INSTALLING) {
    ...
    local_ota_partition = esp_ota_get_next_update_partition(NULL);
    if (local_ota_partition == NULL || image_size > local_ota_partition->size) {
        set_ota_state(OTA_STATE_ERROR, "Firmware image does not fit OTA partition");
        return ESP_ERR_INVALID_SIZE;
    }

    esp_err_t err = esp_ota_begin(local_ota_partition, image_size, &local_ota_handle);
    if (err != ESP_OK) {
        local_ota_handle = 0;
        local_ota_partition = NULL;
        set_ota_state(OTA_STATE_ERROR, "Failed to open OTA partition");
        return err;
    }

    local_ota_image_size = image_size;
    local_ota_bytes_written = 0;
    set_ota_progress(0);
    set_ota_state(OTA_STATE_INSTALLING, NULL);
    power_manager_reset_sleep_timer();
    ESP_LOGI(TAG, "Local OTA upload started: %u bytes -> %s", (unsigned) image_size,
             local_ota_partition->label);
    return ESP_OK;
}

esp_err_t ota_write_local_upload(const void *data, size_t length)
{
    if (local_ota_handle == 0 || data == NULL || length == 0 ||
        local_ota_bytes_written + length > local_ota_image_size) {
        return ESP_ERR_INVALID_ARG;
    }

    esp_err_t err = esp_ota_write(local_ota_handle, data, length);
    if (err != ESP_OK) {
        ota_abort_local_upload();
        set_ota_state(OTA_STATE_ERROR, "Failed while writing firmware");
        return err;
    }

    local_ota_bytes_written += length;
    set_ota_progress((int) ((local_ota_bytes_written * 100) / local_ota_image_size));
    power_manager_reset_sleep_timer();
    return ESP_OK;
}

esp_err_t ota_finish_local_upload(void)
{
    if (local_ota_handle == 0 || local_ota_partition == NULL ||
        local_ota_bytes_written != local_ota_image_size) {
        ota_abort_local_upload();
        set_ota_state(OTA_STATE_ERROR, "Incomplete firmware upload");
        return ESP_ERR_INVALID_SIZE;
    }

    esp_ota_handle_t handle = local_ota_handle;
    const esp_partition_t *partition = local_ota_partition;
    local_ota_handle = 0;
    local_ota_partition = NULL;
    ...
    local_ota_image_size = 0;
    local_ota_bytes_written = 0;
    set_ota_progress(100);
    set_ota_state(OTA_STATE_SUCCESS, NULL);
    ESP_LOGI(TAG, "Local OTA upload validated; next boot partition is %s", partition->label);
    return ESP_OK;
}

void ota_abort_local_upload(void)
{
    if (local_ota_handle != 0) {
        esp_ota_abort(local_ota_handle);
    }
    local_ota_handle = 0;
    local_ota_partition = NULL;
    local_ota_image_size = 0;
