...
static esp_err_t run_internal(wind_app_t *app, bool force_refresh, bool allow_fetch,
                              bool publish_display, bool require_forecast, int64_t now,
    ...
    if (require_forecast && (!local.published_forecast ||
...
esp_err_t wind_app_run(wind_app_t *app, bool force_refresh, int64_t now,
                       wind_app_outcome_t *outcome) {
    return run_internal(app, force_refresh, true, true, false, now, outcome);
}

esp_err_t wind_app_prefetch(wind_app_t *app, bool force_refresh, int64_t now,
                            wind_app_outcome_t *outcome) {
    return run_internal(app, force_refresh, true, false, false, now, outcome);
}

esp_err_t wind_app_run_setup(wind_app_t *app, int64_t now, wind_app_outcome_t *outcome) {
    return run_internal(app, true, true, true, true, now, outcome);
}

esp_err_t wind_app_show_cached(wind_app_t *app, int64_t now,
                               wind_app_outcome_t *outcome) {
    return run_internal(app, false, false, true, false, now, outcome);
}
...
static esp_err_t show_overview_unlocked(size_t page, bool force, bool allow_fetch);
...
static esp_err_t render_dashboard_with_workspace(void *context, const wind_forecast_t *forecast,
    ...
    dashboard->show_tide = display.show_tide &&
        !(runtime->have_tide && runtime->tide.capability == WIND_TIDE_UNSUPPORTED);
...
static esp_err_t show_overview_unlocked(size_t page, bool force, bool allow_fetch) {
    ...
    size_t first = wind_overview_first_spot(page, total);
    ...
    for (size_t row = 0; row < count; ++row) {
        ...
        const wind_display_config_t display = config_manager_get_wind_display_config();
        if (allow_fetch && wifi_manager_is_connected() && display.show_tide)
            load_or_refresh_tide(runtime, force, true, now);
        ...
        wind_app_outcome_t outcome;
        if (allow_fetch) (void)wind_app_prefetch(&runtime->app, force, now, &outcome);
        if (wifi_manager_is_connected()) {
            if (swell) load_or_refresh_swell(runtime, force, allow_fetch, now);
        } else if (swell) {
...
esp_err_t wind_app_show_overview(void) {
    ...
    esp_err_t result=ensure_ready();
    if (result==ESP_OK) result=show_overview_unlocked(s_overview_open ? s_overview_page : s_selected_index/3,false,false);
    xSemaphoreGive(s_runtime_lock);
    return result;
}
...
esp_err_t wind_app_overview_page(int direction) {
    ...
    if (s_overview_open && page>=0 && (size_t)page<=wind_overview_last_page(wind_spots_count()))
        result=show_overview_unlocked((size_t)page,false,false);
    xSemaphoreGive(s_runtime_lock);
    return result;
}
...
esp_err_t wind_app_preview_configuration(const installed_configuration_t *candidate,
    ...
    if (config_manager_set_wind_display_config_transient(&preview_display) &&
        ...
        // Capture while the runtime lock still excludes scheduled fetches.
...
static esp_err_t wind_app_refresh_unlocked(bool force_refresh, bool *published_forecast) {
    ...
    if (s_overview_open && !s_preview_configuration)
        return show_overview_unlocked(s_overview_page, force_refresh, true);
    ...
    ESP_LOGI(TAG, "Forecast refresh: time=%lld attempted=%d fetch=%d freshness=%d displayed=%d unchanged=%d",
             (long long)now, outcome.attempted_fetch, (int)outcome.fetch_result,
             (int)outcome.freshness, outcome.displayed, outcome.display_unchanged);
...
static esp_err_t navigate(int direction, bool absolute) {
    ...
    wind_app_outcome_t outcome;
    result = fetch_before_display
        ? wind_app_run(&runtime->app, !have_cache, now, &outcome)
        : wind_app_show_cached(&runtime->app, now, &outcome);
    if (result == ESP_OK) {
...
bool wind_app_overview_requires_network(int direction) {
    ...
    int page=(int)(s_overview_open ? s_overview_page : s_selected_index/3)+direction;
    size_t count=wind_spots_count();
    xSemaphoreGive(s_runtime_lock);
    if (page<0 || (size_t)page>wind_overview_last_page(count)) return false;
    size_t first=wind_overview_first_spot((size_t)page,count);
    for (size_t i=first;i<count && i<first+WIND_OVERVIEW_PAGE_SIZE;++i)
...
int wind_app_seconds_until_next_wake(void) {
    ...
    if (s_ready && s_selected_index < wind_spots_count()) {
        ...
        next=wind_overview_next_wake(deadlines,count,s_selected_index,
                                     s_overview_open,s_overview_page,next);
    }
