...
#define APP_FORCE_SPLASH_ONLY 0
...
typedef enum {
    DASHBOARD_ACTION_DISPLAY_MODE = 0,
    DASHBOARD_ACTION_NEXT_SPOT,
} dashboard_action_t;

static QueueHandle_t s_dashboard_action_queue = NULL;
static TaskHandle_t s_dashboard_action_task = NULL;

static void dashboard_action_task(void *arg)
{
    (void) arg;
    dashboard_action_t action;
    while (xQueueReceive(s_dashboard_action_queue, &action, portMAX_DELAY) == pdTRUE) {
        esp_err_t result = action == DASHBOARD_ACTION_DISPLAY_MODE
                               ? wind_app_select_next_display_mode()
                               : wind_app_select_next();
        if (result != ESP_OK) {
            ESP_LOGE(TAG, "Dashboard action failed: %s", esp_err_to_name(result));
        } else {
            ha_notify_update();
        }
    }
...
static bool request_dashboard_action(dashboard_action_t action)
{
    if (s_dashboard_action_queue == NULL) {
        s_dashboard_action_queue = xQueueCreate(8, sizeof(dashboard_action_t));
        if (s_dashboard_action_queue == NULL) {
            ESP_LOGE(TAG, "Could not create dashboard action queue");
            return false;
        }
    }
    if (s_dashboard_action_task == NULL &&
        xTaskCreate(dashboard_action_task, "dashboard_action", 24576, NULL, 5,
                    &s_dashboard_action_task) != pdPASS) {
        s_dashboard_action_task = NULL;
        ESP_LOGE(TAG, "Could not start dashboard action task");
        return false;
    }
    if (xQueueSend(s_dashboard_action_queue, &action, 0) != pdTRUE) {
        ESP_LOGW(TAG, "Dashboard action queue is full");
        return false;
    }
    return true;
}
    ...
    while (1) {
        if (BOARD_HAL_WAKEUP_KEY != GPIO_NUM_NC) {
            ...
            if (current_boot_state == 0 && last_boot_state == 1) {
                ...
                if (duration > 50 && duration < 3000) {
                    ESP_LOGI(TAG, "Boot button pressed, enabling firmware maintenance");
                    power_manager_reset_sleep_timer();
                    http_server_enable_maintenance(10 * 60);
                }
            }
        ...
        if (BOARD_HAL_ROTATE_KEY != GPIO_NUM_NC) {
            ...
            if (current_key_state == 0 && last_key_state == 1) {
                ...
                if (duration > 50 && duration < 3000) {
                    ESP_LOGI(TAG, "Left button pressed, cycling display mode");
                    if (request_dashboard_action(DASHBOARD_ACTION_DISPLAY_MODE)) {
                        buzzer_button_acknowledge();
                        power_manager_reset_sleep_timer();
                    }
                }
            }
        ...
        if (BOARD_HAL_CLEAR_KEY != GPIO_NUM_NC) {
            ...
            if (current_clear_state == 0 && last_clear_state == 1) {
                ...
                if (duration > 50 && duration < 3000) {
                    ESP_LOGI(TAG, "Right button pressed, selecting next spot");
                    if (request_dashboard_action(DASHBOARD_ACTION_NEXT_SPOT)) {
                        buzzer_button_acknowledge();
                        power_manager_reset_sleep_timer();
                    }
                }
...
static void wind_dashboard_task(void *arg)
{
    (void) arg;
    while (true) {
        ...
        if (result != ESP_OK) {
            ESP_LOGE(TAG, "Scheduled dashboard refresh failed: %s", esp_err_to_name(result));
        }
    }
}
...
{
    bool is_mode_wake = wakeup_src == WAKEUP_SOURCE_ROTATE_BUTTON;
    bool is_next_wake = wakeup_src == WAKEUP_SOURCE_CLEAR_BUTTON;
    bool is_button_wake = is_mode_wake || is_next_wake;
    // Preserve HA notification and configuration service behavior.
    ...
    bool needs_network = !is_button_wake ||
                         (is_next_wake && wind_app_navigation_requires_network(1));
    if (needs_network) {
        ESP_LOGI(TAG, "Initializing WiFi for forecast refresh");
        ...
        ESP_LOGI(TAG, "Cached spot available; skipping WiFi for button wake");
    }
    ...
    // Bring the config server up before refreshing so HA can still reach the device.
    ...
    if (is_mode_wake) {
        wind_app_select_next_display_mode();
    } else if (is_next_wake) {
        wind_app_select_next();
    } else {
        wind_app_refresh(false);
    }
...
void app_main(void)
{
...
#if APP_FORCE_SPLASH_ONLY
    ESP_LOGW(TAG, "APP_FORCE_SPLASH_ONLY enabled - displaying embedded splash only");
    ESP_ERROR_CHECK(splash_screen_display());
    ESP_LOGW(TAG, "APP_FORCE_SPLASH_ONLY render complete - idling");
    while (true) {
    ...
    if (connect_to_wifi_with_timeout(30)) {
        ...
        ret = wind_app_start();
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Initial dashboard refresh failed: %s", esp_err_to_name(ret));
        }
    } else {
        ESP_LOGW(TAG, "Failed to connect to WiFi - preserving credentials and using cached forecast");
        ret = wind_app_start();
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Offline dashboard refresh failed: %s", esp_err_to_name(ret));
        }
    }

    xTaskCreate(button_task, "button_task", 8192, NULL, 5, NULL);
    xTaskCreate(wind_dashboard_task, "wind_dashboard", 8192, NULL, 5, NULL);
