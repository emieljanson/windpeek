...
TEST(InstallerServiceTest, StateIncludesNumericFailureEvidence)
{
    FakeDevice fake;
    auto service = make_service(&fake);
    service.dependencies.diagnostics = [](void *, wind_installer_diagnostics_t *out) {
        out->apply_error = ESP_FAIL;
        out->http_status = 429;
        out->response_bytes = 128;
        out->device_time = 1787932800;
    };
    const auto state = request(&service, R"({"command":"get_state"})");
    EXPECT_NE(state.find("\"applyError\":-1"), std::string::npos);
...
TEST(InstallerServiceTest, PreservesDeviceAndSpotTimezonesSeparately)
{
    FakeDevice fake;
    auto service = make_service(&fake);
    request(&service, R"({"command":"begin","unixTime":1787932800})");
    ...
    EXPECT_STREQ(service.candidate.spot.timezone, "Europe/Amsterdam");
}
